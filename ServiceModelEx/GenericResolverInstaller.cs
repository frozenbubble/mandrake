...
namespace ServiceModelEx
{
    ...
    {
        private static List<string> webProcesses = new List<string> { "w3wp", "WebDev.WebServer40", "iisexpress" };
        ...
        internal static bool IsWebProcess()
        {
            ...
            return webProcesses.Contains(processName);
        }

        public static Assembly[] GetWebAssemblies()
        {
            ...
            var assemblyInfo = ConfigurationManager.GetSection("Mandrake/Assemblies") as AssemblyLookupConfiguration;
            if (assemblyInfo != null)
            {
                foreach (AssemblyInstanceElement instance in assemblyInfo.Assemblies)
                {
                    assemblies.Add(Assembly.Load(instance.Name));
                }
            }
